...
from lib.models.tools.ray_utils import (
    get_rays_multicam,
    get_rays_at,
...
class NeuS_Trainer(ModuleAbstract, nn.Module):
    def __init__(self, cfg, data=None):
        ...
        self.W = data["W"]
        assert (
            init_focal.shape[0] == 1 or init_focal.shape[0] == 2
        ), f"get wrong focal with size: {init_focal.shape[0]}"
        ...
        self.lambda_pose = cfg.LOSS.get("LAMBDA_POSE", 0.0)
        ...
        logger.info(
            f"{self.name} got include_mask: {self.include_mask} with mask rate: {self.mask_rate}"
        )
        logger.info(
            f"{self.name} got focal order: {cfg.FOCAL_ORDER}, learn_focal: {cfg.LEARN_FOCAL}"
        )
        ...
        logger.info(
            f"{self.name} got lambda_eikonal: {self.lambda_eikonal}, lambda_mask: {self.lambda_mask}"
        )
    ...
    def render(self, c2w, focal, image, img_id, step_idx, mask=None, **kwargs):
        ...
        if self.mask_rate is not None:
            mask_rate = self.mask_rate[0] + (self.mask_rate[1] - self.mask_rate[0]) * (
                step_idx / self.cfg.TRAIN.ITERATIONS
            )
        else:
            mask_rate = None
    ...
    def compute_loss(self, render_dict, **kwargs):
        ...
        if self.lambda_mask != 0:
            mask_loss = F.binary_cross_entropy(
                render_dict["weight_sum"].squeeze().clip(1e-3, 1.0 - 1e-3), render_dict["mask"]
            )
        ...
        if self.lambda_relight != 0:
            ...
            relight_loss = F.mse_loss(
                torch.mean(delta_relight),
                torch.tensor(0, device=delta_relight.device, dtype=torch.float32),
            )
            loss += self.lambda_relight * relight_loss

        if self.lambda_pose != 0:
            pose_loss = self.pose_net.cal_loss(render_dict["cam_ids"])
            loss += self.lambda_pose * pose_loss
    ...
    def validate_image(self, batch, step_idx, **kwargs):
        ...
        rays_o, rays_d = get_rays_at(
            c2w, focal, H, W, normalize=self.normalize_dir, opengl=self.opengl_sys
        )
        ...
        self.SSIM.feed(
            rgb_render.permute(2, 0, 1).unsqueeze(0), image_gt.permute(2, 0, 1).unsqueeze(0)
        )
    ...
    def validate_mesh(self, step_idx, world_space=False, resolution=64, threshold=0.0, **kwargs):
        ...
        vertices, triangles = self.renderer.extract_geometry(
            bound_min, bound_max, device, resolution=resolution, threshold=threshold
        )
    ...
    def testing_step(self, batch, step_idx, **kwargs):
        self.validate_mesh(
            step_idx, world_space=True, resolution=kwargs["recon_res"], threshold=0.0, **kwargs
        )
